import { FaBinoculars, FaSearch, FaSyncAlt } from 'react-icons/fa';
import { FaArrowsRotate } from 'react-icons/fa6';
import { GiWallet } from 'react-icons/gi';
import { ImBinoculars } from 'react-icons/im';

export const TrackingQoutesBody = () => {
    return (
        <>
            <div className="card">
                <div className="card-header">
                    <div className="row mb-2 text-center" >
                        <div className="col-md-1 mb-2"></div>
                        <div className="col-md-3 mb-2">
                            <div className="input-group">
                                <input
                                    type='text'
                                    name='numeroDocumento'
                                    className='form-control'
                                    placeholder='Buscar...'
                                />
                                <button
                                    className="btn btn-primary"
                                    type="button"
                                >
                                    <FaSearch className="iconSize" />
                                </button>
                            </div>
                        </div>

                        <div className="col-md-3 mb-2">
                            <button
                                className="btn btn-success"
                                type="button"
                            >
                                Cambiar Estado  <FaArrowsRotate className="iconSize" />
                            </button>
                        </div>


                        <div className="col-md-3 mb-2">
                            <button
                                className="btn btn-success"
                                type="button"
                            >
                                Abrir Cotización  <FaBinoculars className="iconSize" />
                            </button>
                        </div>
                        <div className="col-md-1 mb-2"></div>
                    </div>

                </div>
                <div className="card-body">
                    <div className="table-responsive-md tablaP">
                        <table
                            className="table table-dark table-hover table-bordered text-md-center">
                            <thead className="table-dark">
                                <tr>
                                    <th>Cotización</th>
                                    <th>Fecha</th>
                                    <th>Nombre Cliente</th>
                                    <th>Total</th>
                                    <th>Estado Actual</th>
                                    <th>Contacto</th>
                                    <th>Teléfono</th>
                                    <th>Número Factura</th>
                                    <th>Monto Factura</th>
                                </tr>
                            </thead>
                            <tbody className="table-secondary">
                                <tr>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                    <td>Test</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                <div className="card-footer bg-secondary">
                    <div className='inline-containerFinal'>
                        <h5 className='tituloFooter'>Total Vendido</h5>
                        <div className="col-md-2 mb-0">
                            <div className="input-group">
                                <span className="input-group-text">
                                    <GiWallet className="iconSize" />
                                </span>
                                <input
                                    type="text"
                                    className="form-control"
                                    placeholder="Total de Ventas"
                                />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>

    )
}
